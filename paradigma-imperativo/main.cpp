...
int counting_letters(string &actual_status_of_word, string &actual_word){
	int count_letters = 0;
	
	for (int i = 0; i < actual_status_of_word.size(); i++) {
		if(actual_status_of_word[i] != '-')count_letters++;
	}
	
	return count_letters;
}

void verify_50(string &actual_status_of_word, string &actual_word, level &actual_level, player &player1, player &player2){
	if(counting_letters(actual_status_of_word, actual_word) >= actual_status_of_word.size()/2){
		
		cout<<"Agora! Você precisa falar a palavra correta!";
		string aux_word;
		
		cin>>aux_word;
		
		if(aux_word == actual_word){
			add_points(player1, actual_level);
			add_lifes(player1, actual_level);
		}else{
			transfer_points(level, player2, player1);
		}
	}
}
...
void update_status_of_word(vector<int> index, string &actual_status_of_word, char letter) {
	...
	if(count_letters >= (actual_status_of_word.size()/2)){
		transfer_points();
	}
}
